...
return new class extends Migration
{
    ...
    {
        // Try a driver-agnostic check using Doctrine if available
        try {
            ...
            // Doctrine not available or incompatible driver (e.g., sqlite in-memory during tests)
            // We'll fall back to attempting to create the index and ignore failures.
        }

        try {
            // Attempt to create a named index in a safe, DB-agnostic way
            // Use CREATE INDEX for broad compatibility, wrapped in try/catch
            $columnList = implode('`, `', $columns);
            DB::statement("CREATE INDEX `{$indexName}` ON `{$table}` (`{$columnList}`)");
        } catch (\Throwable $e) {
            // If index creation fails (index exists or driver doesn't support the statement), ignore
        }
    }
